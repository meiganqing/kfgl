...
var typeArry = ['载体一类型', '载体二类型', '年代', '载体用途','级别','使用用途', '所有权'];
...
if(type){
	for(let i = 0; i < typeArry.length; i++){
	   ...
	   if(typeArry[i] == "所有权" || typeArry[i] == "所属公司") {
        getChartBar(typeArry[i], returnData.rows,"统计内容", "统计值")
	  }else if(typeArry[i] == "使用用途" || typeArry[i] == "载体用途"){
...
//   for(let j = 0; j < carrierType.length; j++){
//     postData("GetTongJiOfDataTable",{
//       tongjiType: "载体二类型",
//       mDataTableName: "拓片资料列表",
//       whereStr: "载体一类型='" + carrierType[j] + "'"
//     },function(data, that){
//       getChart(carrierType[j], data.data, "载体一类型", "tj")
//     })
//   }
