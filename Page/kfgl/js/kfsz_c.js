    ...
    cols = [
        ...
        },
        {
            ...
            templet: function (d) {
                ...
                if (qx[0].Limit.isBJ) {
                    tt += '<a class="layui-btn layui-btn-xs layui-btn-green1" onclick=Edit("编辑","' + d.id + '","' + d.柜架号 + '","' + d.层号 + '")>查看分区</a>';
...
// 删除
function Edit(eventName, mKNBH, fqh, ch) {
    $("#editId").val(mKNBH)
    $("#editfenquhao").val(fqh)
    ...
    switch (eventName) {
        case "编辑":
            ...
            layerPage01 = SysConfig.ToolBox.openWindow("/SYKFGL/Page/kfgl/page/kfsz_fq.html?kf=" + escape(storeCurrent) + "&gjh=" + escape(gjh) + "&gch=" + escape(ch), "查看分区", $(window).width(), $(window).height())
            break;
    
        case "删除":
            SysConfig.SubSystemData.SYKFGL.PLSC([{ id: mKNBH }], '4000', 'DYBH20190823124628462869164', Callback);
            break;
