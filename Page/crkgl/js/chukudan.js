...
var type = window.location.href.getQuery("type");  //ck,rk
var lsh = unescape(window.location.href.getQuery("lsh"));  //记录表流水号
var wwxx_lsh;   //流水号 (待入库列表使用主表的流水号)
console.log(lsh);
...
layui.use(['element', 'table', 'layer', 'form'], function () {
    ...
    if (SysConfig.UserInfo.GetCookieName("kflx") == "拓片") {
        ...
        $("#printBtn").click(function () {
            if(type == "gk"){
                outputFile.OutPutGuiKu("是否打印" + listType + "?", knbh, wwxx_lsh, "", "归库单");
            }else if(type == "ck" || type == "drk"){
                outputFile.OutPutGuiKu("是否打印" + listType + "?", knbh, wwxx_lsh, "", "出库单");
            }
        })
...
function echoData() { //回显数据
    ...
    if (wwxx.success) {
        if(wwxx.rows.length > 0){
            if(type == "drk"){   //待入库列表用主表的流水号
                wwxx_lsh = wwxx.rows[0]['记录表流水号'];
            }else{  //入库流水记录、出库流水记录、待出库都用记录表流水号
                wwxx_lsh = lsh;
            }
